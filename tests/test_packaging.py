...
def test_package_data_assembly(test_stimulus_set_identifier, test_catalog_identifier, brainio_home,
    ...
    package_data_assembly(test_catalog_identifier, assy, identifier, test_stimulus_set_identifier, "DataAssembly",
                          "brainio-temp")
    assert identifier in lookup.list_assemblies()
...
@pytest.mark.private_access
def test_package_data_assembly_without_stimulus_set(test_catalog_identifier, brainio_home,
                                                    restore_this_file, restore_catalog):
    restore_catalog(test_catalog_identifier)
    assy = DataAssembly(
        data=[[[1], [2], [3]],
              ...
              [[6], [7], [8]]],
        coords={
            'stimulus_id': ("presentation", [f"word{i}" for i in range(6)]),
            'stimulus': ("presentation", ["foo"] * 6),
            'subject_id': ("subject", list("ABC")),
            'subject_species': ("subject", ["human"] * 3),
        },
        dims=['presentation', 'subject']
    )
    identifier = "test.package_assembly"
    package_data_assembly(catalog_identifier=test_catalog_identifier, stimulus_set_identifier=None,
                          proto_data_assembly=assy, assembly_identifier=identifier, assembly_class_name="DataAssembly",
